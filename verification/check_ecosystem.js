  ...
  if (text !== null) {
     ...
     * 先に置けば上の段にいるように読める。書かないことを機械で押さえる。 */
    {
      ...
      check('等級の根拠が、表でも散文でもすべて証言である',
        ...
        && text.indexOf('**そして、ここは上がらない。**') >= 0,
        根拠.join(' / '));

      /* **道を示したつもりが、道になっていなかった。**認定状を公開しても
       * `紙面` にはならない —— 紙の画像は誰でも作れる。
       * **見せることと、確かめられることは違う。**そして公開そのものが
       * 決めごと 9 に触れる。**上がる道が無いことを、無いと書く。** */
      check('認定状を公開しないと書いてある',
        text.indexOf('**認定状は公開しない。**') >= 0
        && text.indexOf('決めごとの 9 番に触れる') >= 0);
      check('公開しても紙面にならない理由が書いてある',
        text.indexOf('公開しても `紙面` にはならない') >= 0
        && text.indexOf('紙の画像は誰でも作れる') >= 0
        && text.indexOf('見せることと、確かめられることは違う') >= 0);
      check('上がる道が残っているとすれば何かを書いてある',
        text.indexOf('組織の側が、誰でも確かめられる記録を出しているときだけ') >= 0);

      /* **二つは、金を払って出してもらったものである。**決めごと 14 が
       * 「費用が掛かる時点で、通ったのは選別ではなく支払いである」と書いている。
       * **その物差しは、自分の等級にも当てる。**当てないなら、決めごとのほうが嘘になる。
       * 段級を決めたのが何で、認定状が証しているのが何かを、分けて書く。 */
      check('認定状の出来方を、審査と支払いに分けて書いてある',
        text.indexOf('認定状は受け取っている。だが、四つの出来方は同じではない') >= 0
        && text.indexOf('段級そのものはアプリの対局成績が決めており') >= 0
        && text.indexOf('認定状はその段級に対して申請料を払うと発行される') >= 0);
      check('決めごと 14 を、自分の等級にも当ててある',
        text.indexOf('決めごとの 14 番はここにも掛かる') >= 0
        && text.indexOf('費用が掛かった部分について通ったのは、\n選別ではなく支払いである') >= 0
        && text.indexOf('選別に当たるのは対局成績のほうであって、認定状ではない') >= 0);
      check('アプリと組織の関係が未確認だと書いてある',
        text.indexOf('アプリと組織の関係は、この作業環境からは確かめられない') >= 0
        && text.indexOf('（`未確認`）') >= 0);
      ...
      if (G) {
        ...
        for (const 場 of G['出す場所']) {
          ...
          check('  ' + 場.file + ' が、段の無いこと・年数・支払いを言っている',
            無い.length === 0,
...

{
  ...
  check('出す先と出さない先が、同じ文書に並べてある',
    md.indexOf('## 論文の側は出さない —— 開示が書けない') >= 0
    && md.indexOf('| **道具（リポジトリ）** | **JOSS** |') >= 0
    && md.indexOf('| **論文（プレプリント）** | **どこにも出さない** |') >= 0);
  check('出さないのは費用ではなく開示のためだと書いてある',
    md.indexOf('**費用の問題ではない。**決めごと 14 は、その六篇について障害になっていない。') >= 0
    && md.indexOf('**不完全な開示は、費用より重い。**') >= 0);
  /* **撤回ではない。**棚から動かさないだけである。ここを落とすと、取り下げに読める。 */
  check('取り下げではないと書いてある',
    md.indexOf('**取り下げるのではない。場所を変えないだけである。**') >= 0);
  /* **六篇の内訳が、実際の篇数と合う。**哲学三篇と Trinity-Infinity 三篇である。 */
  {
    ...
    check('「六篇」が、哲学三篇と Trinity-Infinity 三篇の和と合う',
      哲 + ti === 6 && md.indexOf('**だから六篇は、プレプリントの棚に留める。**') >= 0,
