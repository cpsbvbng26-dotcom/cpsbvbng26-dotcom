...

{
  ...
  check('投稿の本文を写していないと書いてある',
    S.indexOf('投稿の本文はここに写さない') >= 0
    && S.indexOf('片方だけ直したときに食い違う') >= 0);
  check('等級が三つに分かれている',
  ...
  check('確かめていない数が、確かめていないと書いてある',
    ...
    && S.indexOf('投稿の中で、数として確かめていないのはここだけである') >= 0);

  /* **評価を事実の欄に入れない。**投稿自身が「と私は思っています」と断っている。 */
  check('評価を事実に混ぜないと書いてある',
    S.indexOf('### 主張ではなく評価である部分') >= 0
    && S.indexOf('この記録は、その評価を事実の欄に入れない') >= 0
    && S.indexOf('相手の性質ではなく自分の側の規則である') >= 0);
  ...
  check('条件 4 と決めごと 14 の積に空きがあると書いてある',
    ...
    && S.indexOf('構造的に格の数字を持てない') >= 0
    && S.indexOf('条件 4 は「数字があるかを見る」と読み替えてある') >= 0);

  /* **原典を読んで足せたものを、足したと書いてあること。** */
  check('原典から足せたことを書いてある',
    S.indexOf('正式名称は arXiv, Inc. である') >= 0
...

{
   ...
   * 差出人は、「同上」で始まらない行の数である。**一通足して見出しを直し忘れると落ちる。** */
  {
    ...
    const m = /表の行はいま(.)つある。差出人は(.)つである/.exec(E);
  ...
  check('四つ目の勧誘が、受理後に掲載料を取ると書いてある',
    ...
    && E.indexOf('費用に触れていない。') >= 0
    && E.indexOf('書かれていないだけである。') >= 0);
  check('免除と金額の不明が、回避の理由にならないと書いてある',
    E.indexOf('免除があることは、規則を回避する理由にならない。') >= 0
    && E.indexOf('金額が分からないことも、理由にならない。') >= 0);

  /* **同じ語が、こちらの探し物と向こうの売り物で一致した。**
   * ここを書かないと、今日の一覧と勧誘の区別が読む側に付かない。 */
  check('通りやすさの出どころの違いを書いてある',
    E.indexOf('`streamlined review process` を売りにしている') >= 0
    && E.indexOf('通ったのは選別ではない。') >= 0
    && E.indexOf('どちらも、金では動かない。') >= 0);
