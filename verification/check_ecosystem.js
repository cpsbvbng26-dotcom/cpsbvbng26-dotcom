...

{
  ...
  if (text !== null) {
    ...
    /* **研究の外の等級は、一つだけ他人が出しているものである。**
     * だから書く価値があり、だからこそ膨らませる余地がある。
     * **膨らませる手は年数のほうである** —— 年数は審査を経ていないのに、
     * 先に置けば上の段にいるように読める。書かないことを機械で押さえる。 */
    check('研究の外の等級を、最初の段だと書いてある',
      text.indexOf('心身統一合氣道 **五級**') >= 0
      && text.indexOf('心身統一道 **初級**') >= 0
      && text.indexOf('どちらも審査の最初の段である') >= 0
      && text.indexOf('上に段があり、そこには達していない') >= 0);
    check('等級が、著者が書いた記録ではないと書いてある',
      text.indexOf('この体系で唯一、**著者が書いた記録ではない**') >= 0
      && text.indexOf('他人が判定して、他人が出している') >= 0);
    check('稽古の年数を書いていない',
      text.indexOf('稽古の年数は書かない') >= 0
      && text.indexOf('年数は審査を経ていない') >= 0
      && !/(合氣道|合気道|稽古|通算)[^。\n]{0,30}[0-9０-９〇一二三四五六七八九十]+\s*年/
            .test(text));
    check('等級の根拠が、証言だと書いてある',
      text.indexOf('免状または審査の記録を指せるようになれば `紙面` に上がる') >= 0);
