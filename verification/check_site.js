...

{
  ...
  /* **自己紹介に貼った写真。**一枚とは限らないので、頁に出ている分を全部拾う。
   *
   * 見るのは五つ —— ファイルがあること、五面が同じ順で同じものを指していること、
   * alt が空でないこと、寸法が実ファイルと合うこと、そして
   * **metadata が残っていないこと**。
   *
   * **Exif は消してから貼る**（決めごと 9）。機種・撮影時刻・ときに位置が入る。
   * JPEG なら APPn、PNG なら eXIf・tEXt・iTXt・zTXt を見る。
   * **差し替えるときに素のまま貼ると、ここで落ちる。** */
  {
    const 面 = ['index.html', 'index.en.html', 'index.de.html', 'index.fr.html', 'index.it.html'];
    const re画 = /<img class="profile-photo" src="([^"]*)" alt="([^"]*)" width="(\d+)" height="(\d+)"/g;
    const 拾い = {};
    面.forEach((f) => {
      const h = read(f);
      const xs = [];
      let m;
      re画.lastIndex = 0;
      while ((m = re画.exec(h)) !== null) xs.push({ src: m[1], alt: m[2], w: +m[3], h: +m[4] });
      拾い[f] = xs;
    });

    /* **五面が同じ順で同じものを指していること。**一面だけ古い写真が残る形を止める。 */
    const 基準 = 拾い['index.html'].map((x) => x.src).join(',');
    const 違う = 面.filter((f) => 拾い[f].map((x) => x.src).join(',') !== 基準);
    ok('五言語とも同じ写真を同じ順で貼っている', 基準.length > 0 && 違う.length === 0,
       違う.length ? ('違う: ' + 違う.join(' / ')) : (面.length + ' 面とも ' + 基準));

    /* 実ファイルを読む。**寸法と metadata は、頁ではなくファイルが持っている。** */
    const 無し = [], 汚れ = [], alt無し = [], 寸法ずれ = [];
    拾い['index.html'].forEach((x) => {
      const rel = x.src.replace(/^\.\//, '');
      const abs = path.join(ROOT, rel);
      if (!fs.existsSync(abs)) { 無し.push(rel); return; }
      ...
      if (b[0] === 0xFF && b[1] === 0xD8) {            /* JPEG */
        ...
        while (i < b.length - 1) {
          ...
          if ((m >= 0xE0 && m <= 0xEF) || m === 0xFE) 汚れ.push(rel + ' の 0xFF' + m.toString(16).toUpperCase());
          if ([0xC0, 0xC1, 0xC2, 0xC3, 0xC5, 0xC6, 0xC7, 0xC9, 0xCA, 0xCB, 0xCD, 0xCE, 0xCF].indexOf(m) >= 0) {
            hh = b.readUInt16BE(i + 5); w = b.readUInt16BE(i + 7);
          }
          i += 2 + ln;
        }
      } else if (b.slice(0, 8).toString('hex') === '89504e470d0a1a0a') {   /* PNG */
        w = b.readUInt32BE(16); hh = b.readUInt32BE(20);
        let i = 8;
        while (i + 8 <= b.length) {
          const ln = b.readUInt32BE(i);
          const tag = b.slice(i + 4, i + 8).toString('latin1');
          if (['eXIf', 'tEXt', 'iTXt', 'zTXt'].indexOf(tag) >= 0) 汚れ.push(rel + ' の ' + tag);
          if (tag === 'IEND') break;
          i += 12 + ln;
        }
      } else { 汚れ.push(rel + ' は JPEG でも PNG でもない'); return; }
      if (x.w !== w || x.h !== hh) 寸法ずれ.push(rel + ' が ' + x.w + 'x' + x.h + '（実 ' + w + 'x' + hh + '）');
    });
    面.forEach((f) => 拾い[f].forEach((x) => { if (!x.alt.trim()) alt無し.push(f + ' の ' + x.src); }));

    ok('貼った写真のファイルが全部ある', 無し.length === 0,
       無し.length ? 無し.join(' / ') : (拾い['index.html'].length + ' 枚とも'));
    ok('貼った写真に metadata が残っていない', 汚れ.length === 0,
       汚れ.length ? 汚れ.join(' / ') : '落としてある');
    ok('貼った写真の alt が、五言語とも空でない', alt無し.length === 0,
 ...
 * Vercel の API が短い間だけ読めて、そのあいだに上の節の断りが三つ崩れた。
 * 読み取りはもう 403 に戻っているので、**同じことを確かめ直す道が無い**。
 *
 * だから固定するのは、読んだ値そのものではなく **表と散文が食い違わないこと** である。
 * 崩れた断りを一行消して数だけ残すと、記録が証拠でなくなる。決めごと 5 である。
 */
section('10.59 読み取りが通った日の記録');

{
  const cs = read('docs/canonical-sources.md');
  const i = cs.indexOf('### 読み取りが通った日');
  const j = cs.indexOf('## この文書が扱わないこと');
  const 節 = (i >= 0 && j > i) ? cs.slice(i, j) : '';
  ok('canonical-sources.md に読み取りが通った日の節がある', 節.length > 0);

  /* 通ったこと自体と、もう通らないことの両方が書いてあること。
   * 片方だけだと「いまも読める」と読める。 */
  ok('読み取りが通ったのが短い間だったと書いてある',
     節.indexOf('通っていたのは短い間で、いまは再び 403 に戻っています。') >= 0);
  ok('設定を触っていないと書いてある',
     節.indexOf('**設定はこちらから触っていません**。読んだだけです。') >= 0);
  ok('上の節を消さないと書いてある',
     節.indexOf('**上の節は消しません**。') >= 0);

  /* **崩れた断りの数を、表から数え直す。**散文の「三つ」と表の行数を突き合わせる。 */
  {
    const k = 節.indexOf('| 上の断り | 読み取りが示したこと |');
    const 行 = k >= 0
      ? 節.slice(k).split('\n').filter((l) => /^\| 「/.test(l))
      : [];
    const 漢 = ['零','一','二','三','四','五','六','七','八','九','十'][行.length] || '?';
    ok('当たらなくなった断りの数が、散文と表で合う（' + 行.length + ' つ）',
       行.length > 0 && 節.indexOf('**' + 漢 + 'つ**が当たらなくなりました。') >= 0,
       '表は ' + 行.length + ' 行');
  }

  /* **読み取りが示した中身。**どれも一行ずつで、消せば落ちる。 */
  ok('GitHub 連携が生きていたと書いてある',
     節.indexOf('| GitHub 連携 | **生きています**。') >= 0);
  ok('閲覧の壁が ssoProtection だと書いてある',
     節.indexOf('| 閲覧の壁 | `ssoProtection` が `enabled`。対象は `all_except_custom_domains` |') >= 0);
  ok('転送が上書きされていると書いてある',
     節.indexOf('転送の配備は、あとの配備に上書きされています。') >= 0);

  /* **知らなかったプロジェクトは、名前で持つ。**数だけにすると、どれが増えたか消える。 */
  {
    const m = /\| 知らなかったプロジェクト \| (.*) \|/.exec(節);
    const 名 = m ? (m[1].match(/`[a-z0-9-]+`/g) || []) : [];
    ok('記録に無かった Vercel のプロジェクトが名前で挙がっている（' + 名.length + ' つ）',
       名.length === 2 && 名.indexOf('`jakusha-dansei-note`') >= 0
       && 名.indexOf('`solitary-school`') >= 0,
       名.join(' '));
  }

  /* **利用者の観察は、等級つきで持つ。**「変わった」を確認と書けば、
   * 確かめていないものを確かめたことにする。決めごと 1 である。 */
  ok('vercel.app が変わったことの根拠が、証言だと書いてある',
     節.indexOf('**根拠は利用者の観察です**（`証言`）。') >= 0
     && 節.indexOf('`vercel.app` の HTML は取れません。') >= 0);
  ok('古い写しが出ていた機構を、確かめていないと書いてある',
     節.indexOf('**その機構は確かめていません**（`未確認`）。') >= 0);

  /* **リポジトリの根に vercel.json が無いこと。**散文がそう書いたので、実際に見る。
   * 置いた日に散文のほうが古くなる。 */
  ok('散文の通り、リポジトリの根に vercel.json が無い',
     !fs.existsSync(path.join(ROOT, 'vercel.json'))
     && 節.indexOf('リポジトリの根に `vercel.json` がありません') >= 0);
}
