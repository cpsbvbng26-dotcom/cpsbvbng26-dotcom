...

{
   ...
   *
   * **2026年9月17日に当てた。**外のプロフィール頁を `陣屋` と呼び、
   * `天守`・`石垣`・`大手門` も README と別のものに割り当てた。
   * README では `陣屋` は trinity-infinity、`石垣` は self-correction である。
   * **散文どうしの食い違いは、数の検査では出ない。**ここで見る。 */
  {
    ...
    ok('食い違えば絵図のほうが誤りだと書いてある',
       cs.indexOf('**絵図が縄張りと食い違えば、絵図のほうが誤りである。**') >= 0);

    /* **城内の部位の語を、城絵図が自分の見出しに使っていないこと。**
     * 使えば、同じ語が二つの意味を持つ。 */
    const 見出し = cs.split('\n').filter((l) => /^#{2,3} /.test(l));
    const 衝突 = 縄.filter((r) => 見出し.some((h) => h.indexOf(r) >= 0));
     ...
     * **数は散文にも書いた。**表から数えて突き合わせる。 */
    {
      ...
      const 有 = (rd.slice(k2, k3).match(/\*\*墨付あり\*\* `10\.5281\//g) || []).length
        + (rd.slice(k2, k3).match(/は墨付あり\*\* `10\.5281\//g) || []).length;
    ...
    ok('README の城外に、飛び地・陣屋の行がある',
       rd.indexOf('| 飛び地・陣屋 |') >= 0);
    ok('陣屋が入口ではなく着地点だと、README にも書いてある',
       rd.indexOf('人を探しに来た者が着いて、そこで止まる場所である') >= 0);
  }

  /* **陣屋の数。**「三つある」と散文が名乗る。**表の行と突き合わせる。**
   *
   * 陣屋と正本は別のものである。混ぜれば、どちらの規則も効かなくなる。
   * ここが見るのは五つ —— 名乗った数と行の数が合うこと、三つの名が並ぶこと、
   * Zenodo が陣屋に入っていないこと（プロフィールの頁が無い）、国の機関でない
   * ものの数が散文と合うこと、そして陣屋が正本に勝たないと書いてあること。 */
  {
    const cs = read('docs/canonical-sources.md');
    const i = cs.indexOf('## 陣屋は');
    const j = cs.indexOf('とも写しである');
    const 節 = (i >= 0 && j > i) ? cs.slice(i, j) : '';
    ok('canonical-sources.md に陣屋の節がある', 節.length > 0);

    /* 特性ごとの表だけを取る。行の頭が ** で始まるものだけが陣屋である。 */
    const k = 節.indexOf('| 特性 | 陣屋 | 運営 | 性格 |');
    const 表 = k >= 0 ? 節.slice(k).split('\n').filter((l) => /^\| \*\*/.test(l)) : [];
    const 漢 = (n) => ['零', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十'][n] || '?';
    ok('陣屋の数が、散文と表で合う（' + 表.length + ' つ）',
       節.indexOf('## 陣屋は' + 漢(表.length) + 'つある') >= 0, '表は ' + 表.length + ' 行');

    ['researchmap', 'HAL', 'PhilPeople', 'SSRN', 'Google Scholar', 'Kudos']
      .forEach((n) => {
      ok('陣屋の表に ' + n + ' がある', 表.some((l) => l.indexOf(n) >= 0));
    });

    /* **面と置き場を分ける。**陣屋は面のほうである。
     * HAL と CV HAL を取り違えると、置き場が陣屋の顔になる。 */
    ok('欧州の陣屋が CV HAL であると書いてある',
       節.indexOf('**欧州の陣屋は CV HAL であって、HAL そのものではない**') >= 0);
    ...
    ok('面と置き場を分ける表がある',
       節.indexOf('| 陣屋（面） | その下の置き場 |') >= 0);
    /* **面と置き場が一つの陣屋。**名前を検査に書き込まない。
     * 散文が並べた名前が、陣屋の表にあることだけを見る。 */
    {
      const m = /\*\*([^*]+?)は、面と置き場が同じ一つである。\*\*/.exec(節);
      ok('面と置き場が一つである陣屋を、名指ししてある', m !== null);
      const 一 = m ? m[1].split(/ と /).map((x) => x.trim()) : [];
      ok('面と置き場が一つの陣屋が、陣屋の表にある',
    ...
    /* **陣屋と計測が同じ手にある例の数。**表から数えて散文と突き合わせる。 */
    {
      ...
      ok('陣屋と計測が同じ手にある例の数が、散文と表で合う（' + 同 + ' 例）',
         節.indexOf('**陣屋と計測が同じ手にある。**' + 漢(同) + '例ある。') >= 0,
         '表は ' + 同 + ' 行');
      ok('営利の陣屋の数と、同じ手が計測を持つ数が揃っている', 同 === 営利,
         営利 + ' / ' + 同);
    }
  }
