 ...
 * 定義が一行のまま動かないこと、覆し方が立っていること、人の名を冠さないこと。 */
{
  ...
  const 定義 = '<b>性愛と家族を持たない男の歴史的範例は、孤立者ではなく共同体の成員だった。'
    + 'ゆえに、その系譜は「独身＝弱さ」を崩すが、「他者に依存しない幸福」は支えない。</b>';
  ok('修道士問題の定義が、一行のまま立っている',
     W.indexOf('<b>修道士問題</b>（The Monk Problem）') >= 0
     && W.indexOf(定義) >= 0);
  /* **片方だけを勝たせる名にしない。**両側への拘束が、表として立っていること。 */
  ok('修道士問題が、両側への拘束を持っている',
     W.indexOf('<th>相手側への拘束</th>') >= 0
     && W.indexOf('<th>こちら側への拘束</th>') >= 0
     && W.indexOf('<b>片方だけを勝たせない名にしてある。</b>') >= 0);
  /* **解消の条件を持たない名は、ここで批判している作りと同じになる。** */
  ok('修道士問題が、解消する条件を三つ持っている',
     W.indexOf('<b>修道士問題が解消する条件は、三つある。</b>') >= 0
     && W.indexOf('<b>先行研究が見つかる</b>') >= 0
     && W.indexOf('<b>この名は、範疇ではない。</b>') >= 0);
  /* **人の名を冠さない。**名を付けたのは他人であるべきで、自分ではない。 */
  const 冠 = ['根本問題', 'Nemoto', '根本の定理', '根本テーゼ']
    .filter((x) => W.indexOf(x) >= 0);
  ok('修道士問題に、人の名を冠していない', 冠.length === 0, 冠.join(', '));
}
