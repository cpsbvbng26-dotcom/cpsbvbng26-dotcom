...

{
   ...
   * **散文どうしの食い違いは、数の検査では出ない。**ここで見る。 */
  {
    ...
    ok('本丸が正本でないと、README にも書いてある',
       rd.indexOf('**六つとも写しである**。') >= 0
       && rd.indexOf('Zenodo と ORCID に合わせて六つの側を直す') >= 0);
     ...
     * **数は散文にも書いた。**表から数えて突き合わせる。 */
    {
      ...
      ok('中枢の四つが両方を欠くと書いてある',
         rd.indexOf('**天守閣・詰丸・石垣・出丸が、四つとも両方を欠いている**。') >= 0);
   ...
   * ものの数が散文と合うこと、そして本丸が正本に勝たないと書いてあること。 */
  {
    ...
    /* **本丸と計測が同じ手にある例の数。**表から数えて散文と突き合わせる。 */
    {
      ...
      ok('本丸と計測が同じ手にある例の数が、散文と表で合う（' + 同 + ' 例）',
         節.indexOf('**本丸と計測が同じ手にある**。' + 漢(同) + '例ある。') >= 0,
         '表は ' + 同 + ' 行');
...

{
   ...
   * 進まないように、等級の一行を消せなくする。** */
  {
    ...
    ok('下書きが、下書きだと名乗っている',
       d.indexOf('**下書き**。まだ公開していない。') >= 0);
    ok('下書きの外の数が、原典未読だと書いてある',
       d.indexOf('いずれも原典を読んでいない') >= 0
       && d.indexOf('検索で確認（原典未読）') >= 0);
    ok('出す前に原典に当たると書いてある',
       d.indexOf('**出すときは原典に当たる。**') >= 0);

    /* **事例が一件しかないことを、隠さない。** */
    ok('事例が一件であることを書いてある',
       d.indexOf('**事例が一件しかない**。') >= 0);

    /* **名指しで非難しない。**法的な線であり、記録の線でもある。 */
    ok('名指しで開示を怠ったと書かないと断ってある',
       d.indexOf('「開示を怠った」と書くことはしない') >= 0);

    /* **この文書自身への跳ね返りを書いてある。** */
    ok('下書き自身の AI 利用を開示してある',
       d.indexOf('この下書きは Claude Code（Anthropic）を使って書いている') >= 0);
  }
   ...
   * **取り消しの一行が消えたら落とす** —— 誤りを消したことになる。 */
  {
    ...
    ok('哲学の出し先が無いと書いたことを、取り消してある',
       節.indexOf('**「Janeway と OLH に哲学の出し先が無い」は誤りだった**') >= 0);
    ...
    ok('旧誌の数字を新誌の格に使わないと書いてある',
       sd.indexOf('**旧誌の数字を新誌の格として使わない**') >= 0);
    ...
    ok('Free & Equal の条件②が未確認だと書いてある',
       sd.indexOf('**②で止まっている**') >= 0
       && sd.indexOf('**「たぶん通る」で押さない**') >= 0);
  }
