 ...
 * **数も頁が名乗っているので、こちらの一覧の長さと突き合わせる**（決めごと 5）。 */
{
  ...
  ok('独我流が、流派ではないと頭で言っている',
     S.indexOf('<b>これは流派ではない。問いの名である。</b>') >= 0
     || S.indexOf('これは流派ではない。問いの名である。') >= 0);
  /* **先行例の前提を落とさない。**先に最上位を得てから興している、という半分を
   * 消すと、都合のいい半分だけが残る。 */
  ok('先行例の前提が、そのまま残っている',
     S.indexOf('<b>こちらは段を一つも持たない。</b>') >= 0
     && S.indexOf('先に既存の系で最上位を得たうえで興している') >= 0);
