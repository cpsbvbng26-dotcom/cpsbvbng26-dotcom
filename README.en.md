...
How published work is counted is fixed as well. Only a degree thesis, or work in a journal ranked Q1 within its field category, or in a peer-reviewed diamond open access journal in the field, goes into the article sections of researchmap and HAL. Everything else is treated as a working paper, a preprint, or MISC. What is on SSRN stays outside that section too. Nothing qualifies at present. The rule above on where work is sent does not change; this one only decides how what appears is counted.
...
**Every push runs 1200 checks.** There are no dependencies to install.
...
node verification/check_site.js      # site structure, 697 checks
