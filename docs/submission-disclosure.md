...
3. **著者自身の筆であることを申告させない**。開示を求める欄と、申告させる欄は違う。
   前者には出す。後者には出さない。
4. **格を先に見る**。索引・区分・順位を、出す前に調べる。2026-09-16 に足した。
   Conatus に出して机上で返されたあとで調べたら、Scopus 収録、
   Arts and Humanities – Philosophy の 854 誌中 197 位、上位四分の一の誌だった。
   **出すなという意味ではない**。返る確率がどれくらいかを、先に知っているかどうかの
   違いである。
5. **置き場所を増やす前に、規約を読む**。同じ本文を複数の場所に置くことを
   咎める場がある。**2026-09-16 に足した**。Figshare で締め出され、
   示された理由が利用規約違反だったためである。どの条項かは分かっていない。
...
| Knowledge Commons | 置き場。**査読は無い** | 未確認。規約を読んでいない | 司会者が次の行き先として挙げた。一篇はすでに置いてある |
...
**誌ではなく、置き場である**。SocArXiv に断られたあと、Knowledge Commons 以外に
どこへ置けるかを当たった。**一つも開いていない**。作業環境から届かないため、
以下はすべて `検索で確認（原典未読）` である。

| 場 | 受ける分野 | 費用 | DOI | 引っ掛かり |
| --- | --- | --- | --- | --- |
| **Jxiv**（JST） | 全分野。人文社会も明文で含む | 無料 | **付く** | researchmap か ORCID が要る。**両方ある** |
| **Preprints.org** | Arts and Humanities に哲学の棚がある | 無料 | 付く | **MDPI の運営**。評判は割れている |
| **PhilSci-Archive** | **科学哲学に限る** | 無料 | 未確認 | 六篇のどれが当たるか、まだ見ていない |

**Jxiv は日本語の稿をそのまま受ける**。SocArXiv が日本語を通さなかったのと逆である。
JST の審査は数日で、査読ではない。

2026年9月17日、利用者が Jxiv を候補から外した。理由は聞いていない。
**こちらで理由を当てない**。外れたという事実だけを書く。
...
大量に来たこと**と、**人文学の司会者がいないこと**を挙げている。

**これは推測ではない**。運営の公開文書に書かれている。ただし**こちらは
`socopen.org` を開いていない**。検索の結果として読んでいる（`検索で確認（原典未読）`）。

**置ける場所は減る側に動いている**。他の人文系の置き場が同じ方向に締まりうる、
というのがここから言えることである。いつどこが締まるかは分からない。

### 増やす前に、規約を読む

**条件 5 がそのまま掛かる**。Figshare で示された理由は「利用規約違反」であり、
Zenodo との重複ではないかというのは利用者の見立てで、確かめていない。

同じ本文の五つ目の写しを作る前に、その場が重複を許すかを見る。
規約の本文はこちらから読めない。

### Open Library of Humanities —— 哲学の誌が三つある（2026-09-18）

**「Janeway と OLH に哲学の出し先が無い」は誤りだった**。2026-09-18 に取り消す。
...
| **Free & Equal: A Journal of Ethics and Public Affairs** | **旧 `Philosophy & Public Affairs`**。2024年に Wiley から OLH へ移った。OLH の年次報告に移行が明記されている | 検索で確認（原典未読）＋利用者が公開頁で確認 |
...
**三つとも掲載料を取らない**。OLH はダイヤモンド OA で、図書館の拠出で回している。
**著者からは取らない**。

**誌の数は確かめていない**。「39 誌」と書いた出典と、「34 誌刊行中・6 誌が加入過程」
とする 2024–25 年の年次報告が食い違う。**どちらが正か決めていない**。
支援する図書館が 345 以上であることは、両方で一致している。

### 旗艦誌には出せない

**`Open Library of Humanities` 誌（旗艦誌）は一般投稿を受け付けていない**。

> The journal is now dedicated to publishing Special Collections … and so
> no longer accepts general submissions.

**ここを取り違えていた**。「旗艦誌が人文学全域を受ける」と書いていたが、
受けていない。**出すのは個別誌である**。
...
始まるため、創刊直後の誌には原理的に付かない。**数字は抜けられた側に残っている**。

**「旧誌が最上位だから格が高い」と書きかけた**。取り消す。
**旧誌の数字を新誌の格として使わない**。

### 条件 4 が、決めごと 14 と噛み合わない

条件 4 は「格を先に見る」である。**調べた結果、数字が無いことが分かった**。
これは格が低いことではない。**測る道具が届いていないだけである**。

**ここで二つが噛み合わなくなる**。決めごと 14 が向かう先はダイヤモンド OA の
場であり、その多くは新設誌である。**新設誌は構造的に数字を持たない**。
数字が無い場を外せば、決めごと 14 が勧める場をほとんど外すことになる。

**条件 4 を「数字を見る」から「数字があるかを見る」に読み替える**。
無いときは、無いと書いて出す。**推定を確定として書かない**。
...
| | 判定 | 根拠 |
| --- | --- | --- |
| ① 掲載料・投稿料を取らない | **満たす** | OLH がダイヤモンド OA である。明文 |
| ② 公開済みのプレプリントを除外していない | **未確認** | **公開頁からは読み取れなかった** |
| ③ 著者自身の筆を申告させない | **未確認** | 投稿規程を読む必要がある |
| ④ 格を先に見る | **数字が無いことを確認した** | 上のとおり |
| ⑤ 規約を読む | **未確認** | 同上 |

**②で止まっている**。三篇は Zenodo・SSRN・PhilArchive に置いたままにする。
**置いたままで出せるかが分からない以上、出さない**。

**「たぶん通る」で押さない**。投稿画面の丸一つが、数週間を決める。

**分野の合い方は明確である**。Free & Equal は対象として philosophy・public
...
**倫理・自律・公共的な問題として組み直せるかが効く**。一般の哲学誌ではない。
...
| Figshare | **締め出された**（`証言`）。示された理由は利用規約違反。どの条項かは分かっていない。索引の穴を埋める先としては道が無い |
...
**格の数字を見ないと決めて調べた**。条件 4 は残っているが、この回はそれを先に見ていない。
七篇を査読に掛けることのほうを先に置いた。条件 1・2・3 は外していない。

**同じ稿を二つの誌に同時に出さない**。この決めは動かない（「気をつけること」の節）。
七篇を別々の誌に一篇ずつ出す形でだけ、全部を出せる。

### 通りやすさの出どころは二つある

**金で通りやすい場は、この表に入れていない**（決めごと 14）。残るのは二つである。

| 出どころ | 何が効いているか |
| --- | --- |
| **母集団が狭い** | 学部生の誌。競争の相手が学部生に限られる。**在学中だけの資格である** |
| **射程が合う** | 新規性ではなく、経験・歴史・哲学の側を採る誌。数学三篇の貢献 0 が門にならない |

### 学部生の誌 —— 五つ。ZEN大学に在学しているあいだだけ出せる

哲学三篇の受け皿である。査読があり、費用は無い。卒業すれば資格が消える。
...
**数学三篇は、どの数学研究誌にも出せない**。紙面が自分で「新しい数学は無い」と
書いている。新規性の門に当たらない。射程のほうを変えるしかない。

| 誌 | 出している所 | 射程 |
| --- | --- | --- |
| Journal of Humanistic Mathematics | Claremont Colleges Library | 数学の美的・文化的・歴史的・哲学的な側面。掲載料も投稿料も無い。DOAJ 収録 |
| The Mathematics Enthusiast | University of Montana | 学部上級で読める数学の内容、歴史・哲学・異文化。所属を問わない。投稿から掲載まで 8〜12 か月 |

Journal of Humanistic Mathematics が本命である。 分野の既知を増やしていないことが、
ここでは欠格にならない。`ROUTE.md` の九段階は、この誌の射程の中にある。

### 一般のダイヤモンド哲学誌 —— 七つ。金は取らないが、通りやすくはない
...
| 『日本歴史』 | 査読誌だが、日本歴史学会の会員に限る。会費は掲載料ではない。決めごと 14 に触れるとは限らない。境界である |
| 『史学雑誌』 | J-STAGE でフリーアクセス。投稿の条件は未確認 |
| 数学の研究誌 | 新規性の門。紙面自身が貢献 0 と書いている |
| 勧誘してきた出版社 | 決めごと 14 |

史料ノートの受け皿が見つかっていない。 日本史の査読誌は会員制が多い。
ここだけ空いている。

### 出す前にやることが三つある

1. `E8` の帰属を先に直す。 重大度・高で立っているあいだ、Series I〜III の
   `This paper's contribution is Theorem 1` は帰属の側で立たない。
   既知の模型の特殊例を自分の貢献として述べたまま査読に出すことになる。
2. **AI 支援の開示を消さない**（決めごと 8）。道具の名・版・使い方を書けば認める形が
   主流になっている。開示ありで通す。
3. **プレプリントの可否を誌ごとに確かめる**（条件 2）。六篇は Zenodo にあり、動かさない。

### 確かめた度合

全部が検索で確認であり、原典を読んでいない。 作業環境から届くのは GitHub だけである。
`openjournals.bsu.edu` は egress proxy が塞いだ。上の条件は、投稿前に原文で確かめる。

---

## X に書いたこと —— 2026-09-18

著者が X に投稿した文である。置き場の三つと、プレプリントが出版の何を先取りするかを書いた。
この節は、その中の事実の主張だけを取り出し、等級を付けたものである。

投稿の本文はここに写さない。写せば同じ文が二か所になり、片方だけ直したときに食い違う。
記録するのは、確かめられる部分と確かめていない部分の切り分けである。
...
投稿の中で、これがいちばん強く疑われた部分である。ChatGPT は 7 月 1 日の一点を
「公式ソースを確認できるまで保留」と述べた。確かめたところ、arXiv 自身の文書に書いてあった。
`arxiv.org` は egress proxy が塞いでいるが、arXiv が GitHub に置いた文書には届く。

原典を読んで足せることが二つある。正式名称は arXiv, Inc. である。
デラウェア州の非営利・無株式法人で、設立時の Member は Simons Foundation と
Cornell University の二者である。
...
投稿の中で、数として確かめていないのはここだけである。

### 主張ではなく評価である部分

「ハゲタカが売っているのは業績欄の一行だけ」は評価である。投稿の本文も
「と私は思っています」と断っている。この記録は、その評価を事実の欄に入れない。

決めごと 14 が書いているのは、相手の性質ではなく自分の側の規則である。
費用が掛かる時点で、通ったのは選別ではなく支払いである。これは相手を判定しない書き方で、
同じことを言っている。

### 記録の側と噛み合っているところ

投稿は、出す先を「Q1・Q2 か、著者にも読者にも課金しないダイヤモンド OA」に絞ると書いた。
記録の側では、条件 4 が格を先に見ると定め、決めごと 14 がダイヤモンド以外を外している。
二つの条件の積が、投稿の言う絞り方と同じものになる。

ただし記録の側は、その積に空きがあることも書いている。新しいダイヤモンド誌は
構造的に格の数字を持てない。Open Library of Humanities の三誌がその例である。
条件 4 は「数字があるかを見る」と読み替えてある。

### 副作用について

投稿は副作用を三つ挙げた。基準が上がって出版が遅くなること、媒体の格への依存が強まること、
投稿規定の確認が必須であること。三つ目は条件 2 と条件 5 が既に同じことを定めている。

二つ目は、この記録の `ST-002` が塞いでいる側と同じ形である。検査の数も媒体の格も、
中身の価値について何も言わない。
