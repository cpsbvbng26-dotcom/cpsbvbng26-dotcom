...
何も返らなかったことも、その場について分かることの一つである。
...
**これが査読誌への最初の投稿である**。それまでは、査読の付く場に一篇も出していなかった。
...
**出したのは一篇だけである**。枠が一つしかない。選んだ理由は
[`submission-disclosure.md`](submission-disclosure.md) に書いてある。

**添えたもの**。開示文を編集者へのコメントに貼った。中身は三つである。
...
**原稿は公開版と一字一句同じである**。体裁だけを誌の規程に合わせた。
落としたのは扉の著者行と書誌の表と日本語の要旨で、いずれも本文ではない。
`autonomy-and-self-cultivation/verification/check_manuscript.py` が突き合わせている。

**AI 支援の開示は原稿の中に残してある**。著者を指さないので、匿名の査読でも残せる。

### 結果 —— 落ちた

**返ってきた決定は `Decline Submission` である**。運営編集者の名で届いた。

> Our decision is to: Decline Submission

**理由は示されていない**。査読の所見も、どの条件で外れたかも、書かれていない。
**分かっているのは、落ちたということだけである。**

**査読には回っていないと見てよい**。出してから決定まで一日である。
双方向匿名の査読を二名に依頼して所見が戻る時間ではない。
**ただしこれはこちらの推測であって、誌はそう述べていない。**

**理由を当てにいかない**。開示か、所属か、プレプリントか、中身か。
候補は挙げられるが、どれも確かめていない。**候補を書けば、確かめたものと
混ざる危険のほうが大きい。**

### Figshare から締め出された

**利用者から受け取った報せである**。Figshare で締め出された。
...
**利用者は Zenodo との重複ではないかと述べている。これは推測である。**
Figshare がそう述べたのではない。**条項の特定には至っていない。**
推測を理由の欄に上げない。**上げれば、示された理由と区別が付かなくなる。**

**作業環境から `figshare.com` へ出られない**。規約の本文も、締め出しの通知も、
こちらでは読んでいない。

**それでも書く**。落とされた先を書かずに置けば、置き場所の一覧は
「どこにでも置ける」ように見える。**置けない場所があることは、置けなかった側にしか示せない。**
SSRN と Conatus の行と同じ扱いである。

**Figshare は候補から外れた**。索引の穴を埋める先として挙げていたが、道が無い。

### 重複そのものが咎められうる

**推測が当たっているかどうかとは別に、ここで一つ分かったことがある。**
**同じ本文を置き場所ごとに増やす手が、規約の側から咎められる場合がある。**

この記録は同じ本文を四か所に置いている —— Zenodo、SSRN、PhilArchive、
Knowledge Commons。**そのどれも、いままで咎められていない。**
だが咎めない保証がどこにあるかは、確かめていない。

**次に置き場所を増やすときは、先に規約を読む**。Conatus のときに
「格を先に見る」を足したのと同じ形である。**増やす前に、増やしてよいかを見る。**

### 出した誌がどこにあるか

**落ちたあとで調べた**。先に調べておくべきだった。
...
**掲載料も投稿料も取らずに Scopus に入っている**。この二つが揃う誌は多くない。
決めごと 14 は「費用が掛かる時点で、通ったのは選別ではなく支払いである」と書いている。
**ここは逆の側である**。費用がゼロで、選別が実在する。

**つまり、最初の投稿で分野の上位四分の一に出したことになる。**
**落ちたことは、そのことと矛盾しない。**

**これは検索で確認したものである。原典には当たっていない。**
`scimagojr.com` も `miar.ub.edu` も、この作業環境から届かない。
**採択率は公表されていない**。探したが出てこなかった。

### 通った場合に動くはずだったもの

**この場所は「公開しているものは、いずれも査読を受けていない」と何箇所かで書いている。**
一篇でも通れば、その文が偽になる。**通ってから直すのでは遅いので、先に洗い出してあった。**

**今回は動かさない**。その文はまだ真である。洗い出しは次に持ち越す。
...
**日付も記録していない**。確かなのは一点である。通った二篇と同じ門に出して、
同じ扱いにならなかった。

**この行を消さない**。通った二篇だけを並べると、門が何も落とさないように見える。
落とす門であることは、落とされた側にしか示せない。

**落とされた行は二つになった**。SSRN と Conatus である。
...
**人の側の区分は、あとから画面で確かめている。区分は non-pro である。**

`Find a Philosopher` を、non-pro を**含める**設定にすると、所属（`ZEN大学`）で引いて出る。
**出るのは、その設定のときである**。名前では出てこない。
**所属で出たことは、区分を覆さない**。分けているのは設定のほうである。
...
**名指しされたのは史料ノートである。** 三つ目と同じものである。
**この記録は、それを論文と呼んでいない。** 文面は `your research` と書いている。
...
**費用に触れていない。** 一つ目から三つ目までと違い、**無償も謳っていない。**
**書かれていないだけである。**

#### 調べたのは、掲載料の有無だけである

**順序を先に書く。決めたのが先で、調べたのは後である。**
調べたのは掲載料の有無に限る。**決めごと 14 が掛かるかどうかが、そこで決まるからである。**
**相手の性質を調べたのではない。**
...
**作業環境から誌の頁に届かない。** 上はすべて検索結果から取ったものである。

#### 決めたこと —— 応じない

**決めごと 14 が「掲載料・投稿料を取る場には出さない」と定めている。** そこで終わる。

**免除があることは、規則を回避する理由にならない。**
免除を求めること自体が、選別の外で費用の話をすることである。
**金額が分からないことも、理由にならない。** 取るかどうかで決まる規則である。

**これは相手についての判定ではない。** `Eliva Press` の欄と同じ扱いにする。
実在も実績も査読の質も確かめていない。**決めたのは自分の側の規則である。**

#### 一つだけ、他の三つと違うところがある

**文面が `streamlined review process` を売りにしている。**

同じ日、こちらは掲載料を取らない誌を「通りやすさの順に」並べていた
（[submission-disclosure.md](submission-disclosure.md)）。
**こちらの探し物と、向こうの売り物が、同じ語で一致している。**

**だが決めごと 14 がそこを切る。** 通りやすさを金で買えるなら、
**通ったのは選別ではない。** 決めごと 14 の後段が書いているのはそれである。

**そして、こちらが並べた場との違いは、通りやすさの出どころにある。**
学部生の誌が通りやすいのは母集団が狭いからで、人文数学の誌が通りやすいのは
射程が合うからである。**どちらも、金では動かない。**

### 登録の審査 —— **中身の審査ではない**

**researchmap の登録は、誰かの推薦で入ったものではない。**
**HAL に登録したプレプリントを業績として出し、審査を通っている。**
...
- **審査の記録は、こちらから見えない**。通知も判定理由も公開されていない。
- **HAL の登録そのものも、この作業環境から確かめていない**（`hal.science` に届かない）。
  検索でも、同姓同名の別人しか出てこない。**書いてあるのは著者から受け取ったとおりである。**
...
**通ったのは登録の資格であって、書いたものの質ではない。**
SSRN や PhilArchive の欄と並べて読むときも、そこは分けて数える。

**それでも書くのは、経路が違うからである**。推薦で入ったのと、
業績を出して審査を通ったのとでは、**同じ登録でも意味が違う。**
**そして前者だと誤解される余地がある。**
...
**出版社からの勧誘が届いている**。評価ではないので、「受けた評価」には入れない。
**だが、届いたことは記録する**。都合の悪い接触だけ落とせる状態にしない。

**表の行はいま六つある。差出人は四つである。**
...
**題名は、現行の紙面と完全に一致する**。`autonomy-and-self-cultivation/CITATION.cff` の
`title` と、引かれた文字列が一字も違わない。**古い版から拾ったものではない。**
機械で突き合わせた。

**それ以外は、何も確かめていない**。この作業環境から出版社に届かない。
**実在・実績・査読の有無・契約の内容、いずれも `未確認` である。**

### 出版社について、調べて分かったこと

**順序を先に書く。決めたのが先で、調べたのは後である。**
決めごと 14 は相手の性質に依らない規則なので、**調べた結果がどうであれ、決定は変わらない。**
以下は、**記録として残すために調べたもの**である。

| 項目 | 分かったこと | 等級 |
| --- | --- | --- |
| **国** | モルドバ共和国・キシナウ。`Eliva Press SRL` として所在地を公表している | **検索で確認**（本人の公表） |
| **言語** | 英語。サイトも勧誘も英語である。刊行言語の一覧は取れていない | **検索で確認** |
...
**団体の会員としては見つからない**。COPE（出版倫理委員会）にも
OASPA（オープンアクセス学術出版協会）にも、会員として現れない。
**ただしこれは、会員でないことの証明ではない**。見つからなかった、というだけである（**検索で確認**）。

**索引への収録も確認できない**。Scopus・Web of Science に刊行物が入っているかは、
確かめられなかった（**未確認**）。

**一覧に載っているという報告がある**。いわゆる Beall's List の
`vanity press` の一覧に 2021年12月に追加された、と検索結果が述べている。
**こちらは一覧そのものを読んでいない**（作業環境から届かない）。
**そしてこの一覧自体が、学術界で争われているものである** ——
作成者が一度取り下げ、以後は匿名で維持されている。**一覧に載っていることは、
一覧を作った側の判断であって、確定した事実ではない**（**未確認**）。

**利用者の報告は割れている**。肯定的な体験談も、否定的な体験談も、
どちらも公開の場に出ている（**未確認**）。

**この記録は、どちらとも判定しない**。判定できる材料を持っていないからである。
**並べたのは、出典と等級だけである。**

### 決めたこと —— 応じない

**決めごとの 14 番が、この形をそのまま名指ししている。**
...
**同じ相手から、追いかけの一通が来た**。2026年9月である。
名指しされている著作は一通目と同じ `Manifesto of Imperial Selfhood` である。
...
**新しい条件は何も付いていない**。返事が無いことへの催促だけである。
**こちらの状態は変わらない**。返信していない。

**追いかけが来ること自体は、一つのことを示している。**
一通目が個別の判断で送られたのなら、返事が無い相手を追う手間は割に合わない。
**追う仕組みがあるということは、送る仕組みもあるということである。**
ただし**これは推測であって、確かめていない。**

**個人名は記録しない**。一通目と同じ扱いにする。
...
**文面はそこに触れていない**。触れていないことは、文面から確かめられる。

**そこから先は推測である**。読んだうえで触れなかったのか、読んでいないのかは、
**こちらには分からない**。どちらであるかを述べる材料を持っていない。
`Eliva Press` の欄と同じ扱いにする。
...
**勧誘は、通過の記録ではない**。査読でも受け付けでもなく、**選別を一つも経ていない。**
「著作が印象的だった」と書かれていても、それは評価の記録にならない。

**だが、載せないと「来なかった」と区別が付かない**。落ちた記録を同じ欄に置いてあるのと
同じ理由で、**応じなかった記録も置く。**
...
**これは評価ではない**。GitHub が自動で出す印で、**マージされた PR の数を数えたもの**である。
中身についても、正しさについても、何も言わない。査読でも受け付けでもない。

**数えている PR は、自分の repository のものである**。このリポジトリの閉じた PR を
API で見た。**出したのは本人の名義である**（`cpsbvbng26-dotcom`）。
**通した側は見ていない**。協力者の一覧を確かめていないため、誰がマージしたかは
分からない。**単独の持ち主の repository で、外から通した者がいたとは書かない。**

**段も書かない**。Pull Shark には数に応じた段があるが、どの段かは受け取っていない。

**紙面には置けない**。公開サイトの CSP は `default-src 'none'` である。
GitHub のバッジも外部へのリクエストになる。PlumX のウィジェットと同じ扱いになる。
...
researchmap を本丸だと思うはずである。**

**当たっている**。そして、顔をしているだけではない。**運営は国の機関である。**
...
**器の格は本物である**。だから本丸に見えるのは当然で、**だから食い違いの代償が大きい。**

**器は国の機関。中身は自己申告である**。この二つは別で、
上の「登録の審査 —— 中身の審査ではない」がその区別を書いている ——
researchmap が見たのは登録要件であって、論文の中身ではない。

**権威があるのは登録の資格の側で、書いたものの質の側ではない。**
**そして国の機関が運営しているぶん、外から来た人は後者にも権威が及んでいると読む。**

そのうえで、researchmap は**写しである** —— 内容は Zenodo と ORCID から写したものである。

**写しが本丸に見える構えは、食い違いが起きた瞬間に壊れる**。researchmap 側に
古い記述が一つ残れば、外から来た人はそれを正として読む。**Vercel が古い科目名を
出していたのと同じ形である**。あちらは写しが入口に立っていた話で、
こちらは写しが本丸に見えている話である。

**上の節に足りていなかったのは、この軸である**。そこに書いてあるのは
DOI が割れている話だけで、**どこが本丸に見えるかは書いていなかった。**

**規則は既にある。表示が無い。**

> これらに食い違いが見つかったときは、Zenodo と ORCID に合わせて外部側を直す。逆はしない。

**researchmap 自身が「これは写しである」と言っていない**。HAL も PhilPapers も
同じである。**出丸が三つあって、どれも自分が出丸だと名乗っていない。**

**直すのは利用者の手になる**。作業環境からどこへも届かない。各頁の自己紹介か
備考に、正本の所在を一行入れる形になる。**まだ入れていない。**
...
人を探しに来た者は、そこに着いて、そこで止まる。**入口ではなく着地点である。**
...
**この二つを分けたので、「写しが本丸に見える」はもう矛盾ではない。**
