...
**人の側の区分は、あとから画面で確かめている。**

`Find a Philosopher` を、non-pro を含めない設定で引いた。**名前では出てこない。**
所属（`ZEN大学`）で引いたときだけ候補の箱に出た。**候補の箱と一覧は、同じ索引を見ていない。**
**採るのは、名前で出ないほうである。**既定の一覧が落とすのは non-pro の側だからである。
**所属で一件出たことは、区分を覆さない。**
