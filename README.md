...
外から下された判定は、研究の外にもある。心身統一合氣道 五級、心身統一道 初級（心身統一合氣道会）、囲碁 十九級（日本棋院）、将棋 九級（日本将棋連盟）。四つとも級である。段は一つも無い。ここに出しているのは、数を並べるためではない。この場所で唯一、著者が書いた記録ではないからである —— 検査も登録簿も経路も、すべて著者が書いて著者が走らせている。等級だけが他人の判定を経ている。稽古や対局の年数は書かない。年数は審査を経ていない。根拠はすべて著者の証言であり、免状または認定状を指せるようになるまでそのままである（自己分析）。
...
node verification/check_ecosystem.js  # 9 リポジトリ横断 277 項目
