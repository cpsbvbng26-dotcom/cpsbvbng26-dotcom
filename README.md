...
**いま一番の主張。** 公開された成果物は、もう直せません。直せるのは訂正のほうです。**だから訂正は、時間とともに元の資料からずれていきます。**引用が一字変わり、箇所を数え落とし、未解決と決めた項目が「解決済み」に書き換わる。**そのずれは、推論を使わずに機械で落とせます。**LLM も類似度も使いません。あるか、無いか、一致するか、しないか。だから出力を人が確かめ直す必要がありません。
...
```
node verification/check_all.js        # 9 リポジトリの検査を全部（27 本）
node verification/check_ecosystem.js  # 9 リポジトリ横断 45 項目
```
