...
> **免責。** `EX` から `F` の並びは**学術の世界に存在しない。遊戯の等級表の形式を借りたもの**で、
> 形式は著者が持ち込んだものである。**動かないのは通過と却下の事実だけ**（査読 **0 篇**、
> SSRN **3 通過 1 却下**、PhilArchive **2 通過**。いずれも機械が数え直している）。
> **段をいくつに切るか、境目をどこに置くかは著者が決めた。**同じ事実から違う段割りも作れる。
> **絶対ではない。**分野ごとに門の作りが違うので、**普遍でもない。**
> **他人を測る物差しでもない** —— ここに並ぶのは著者一人の公開物についての段であり、
> **他の誰も、この表に載ることに同意していない。**他人を当てて上下を言うために作った
> ものではなく、そう使われるなら誤用である。**価値については何も言わない**（登録簿の `ST-002`）。
> それでも置いてあるのは、**段が他人の通過でしか上がらない形にしてあるから**である。
...
| [self-correction](https://github.com/cpsbvbng26-dotcom/self-correction) | **自分が公開した主張のうち、誤っていたもの・撤回したもの・直せないものを、一件ずつ消さずに記録する。**いま 86 件。いま立っている主張には覆し方を、直せない項目には理由を書くことを検査で強制する。**識別子は永久に消せない** —— git の履歴を遡り、過去に一度でも載った項目が消えていれば落ちる | MIT | — |
...
node verification/check_ecosystem.js  # 9 リポジトリ横断 251 項目
...
**`check_ecosystem.js`** が見るのは三つである。散文が名乗る数（「道具自身 74」「登録簿 86 件」「壊す先 28 通り」など）が**実際に走らせた数と一致するか**。足し算で名乗っている数（正誤表の監査 132 = 51 + 50 + 31）が**足した結果と一致するか**。そして写した `errata_check.py` の版と、そのリポジトリが書いている DOI が**対応しているか**（`trinity-infinity` だけ v0.1.0 を写しているので、DOI も別の番号になる）。
...
| 検分状 | [`docs/self-assessment.md`](docs/self-assessment.md)。日付つきの現状評価と相対ティア（**数学 C / 哲学 C / 歴史 D / 総合 D**）。段は `papers.json` から計算する。**質の等級は書けない**（登録簿の `ST-002`） |
