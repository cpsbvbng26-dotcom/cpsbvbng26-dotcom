...
外から下された判定は、研究の外にもある。心身統一合氣道 五級、心身統一道 初級（心身統一合氣道会）、囲碁 十九級（日本棋院）、将棋 九級（日本将棋連盟）。四つとも級である。段は一つも無い。ここに出しているのは、数を並べるためではない。この場所で唯一、著者が書いた記録ではないからである —— 検査も登録簿も経路も、すべて著者が書いて著者が走らせている。等級だけが他人の判定を経ている。稽古や対局の年数は書かない。年数は審査を経ていない。認定状は受け取っている。だが、四つの出来方は同じではない。合氣道の二つは審査である。囲碁と将棋の二つは、段級そのものをアプリの対局成績が決めており（囲碁シル、将棋ウォーズ）、認定状はその段級に対して申請料を払うと発行される。費用が掛かった部分について通ったのは、選別ではなく支払いである。選別に当たるのは対局成績のほうであって、認定状ではない。根拠はすべて著者の証言である —— 認定状は手元にあるが、公開していない以上、誰にも確かめられない（自己分析）。
...
| [self-correction](https://github.com/cpsbvbng26-dotcom/self-correction) | **自分が公開した主張のうち、誤っていたもの・撤回したもの・直せないものを、一件ずつ消さずに記録する。**いま 90 件。いま立っている主張には覆し方を、直せない項目には理由を書くことを検査で強制する。**識別子は永久に消せない** —— git の履歴を遡り、過去に一度でも載った項目が消えていれば落ちる | MIT | — |
...
node verification/check_ecosystem.js  # 9 リポジトリ横断 334 項目
...
**`check_ecosystem.js`** が見るのは三つである。散文が名乗る数（「道具自身 74」「登録簿 90 件」「壊す先 28 通り」など）が**実際に走らせた数と一致するか**。足し算で名乗っている数（正誤表の監査 132 = 51 + 50 + 31）が**足した結果と一致するか**。そして写した `errata_check.py` の版と、そのリポジトリが書いている DOI が**対応しているか**（`trinity-infinity` だけ v0.1.0 を写しているので、DOI も別の番号になる）。
